...
chrome.storage.sync.get(['textLang', 'translateLang', 'etranslateExtensionKey'], function(result) {
...
document.addEventListener('mouseup', function(e){
    ...
    if (selection.toString() !== ''){
        ...
        if (arrStr.length < 11 && arrStr[0] !== ''){
            ...
            if (!tt){
                fetch(`https://safe-castle-97193.herokuapp.com/api/extension-translate?key=${etranslateExtensionKey}&textLang=${textLang}&translateLang=${translateLang}&text=${selection.toString()}`)
                .then(res=> res.json())
                .then(text => {
