chrome.storage.sync.get(['etranslateExtensionKey'], function({ etranslateExtensionKey }) {
    ...
    chrome.storage.sync.get(['textLang', 'translateLang'], function({ textLang, translateLang }) {
        if (textLang){
        ...
        if (translateLang){
            ...
            chrome.storage.sync.set({['translateLang']: selectTranslateLang.value});
        }
    });
    ...
    if (etranslateExtensionKey){
        ...
        input.addEventListener('keypress',e => {
            if (e.keyCode === 13){
                fetch(`https://safe-castle-97193.herokuapp.com/api/get-extension-key`,{
                    method:'POST', 
                    headers: {
                    ...
                    .then(res=> res.json())
                    .then(data => {
                        chrome.storage.sync.set({['etranslateExtensionKey']: data.id});
